...
public class ShrygmyTaskManager<T extends Shrygmy> extends TaskManager<T> {
    /**
     ...
     */
    public ShrygmyTaskManager(T mob, Brain<T> dynamicBrain) {
    ...
    protected void initWorkActivity(int priorityStart) {
        dynamicBrain.addActivity(Activity.WORK, priorityStart, ImmutableList.of(StartAttacking.create(ShrygmyTaskManager::findNearestValidAttackTarget), createIdleLookBehaviors(), createWorkMovementBehaviors()));
